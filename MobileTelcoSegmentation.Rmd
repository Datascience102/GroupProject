...
Now we replace all initial variables used in this part with the factors scores. The factor scores look like the following for the first customers in our dataset:
...
cluster_profile_ratios <- (ifelse(population_average_matrix==0, 0, ifelse(abs(Ratio_result)>=1, 1, Ratio_result)))

colnames(cluster_profile_ratios) <- paste("Seg.", 1:ncol(cluster_profile_ratios), sep="")
rownames(cluster_profile_ratios) <- colnames(NEW_ProjectData)
...
**Segment Enterprise**
<br>
Strong in several services that enterprise clients use: offnet and international SMS, voice packages, value added services (VAS) packages, IVR short code (shorter number, with less digits), on Net voice and international voice
<br>
<br>
**Segment Short-term client**
<br>
Active for less than 20 days
<br>
<br>
**Segment Family abroad**
<br>
Strong in out roaming and on airtime (credit) gifts
